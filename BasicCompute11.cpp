...
#define NUMBER_MATRIX 10

struct BufType
{
	DirectX::XMFLOAT4X4 f[NUMBER_MATRIX];
};
...
int __cdecl main()
{
    ...
    for ( int i = 0; i < NUM_ELEMENTS; ++i ) 
    {
		...
		{
			g_vBuf0[i].f[j] = DirectX::XMFLOAT4X4();
			g_vBuf1[i].f[j] = DirectX::XMFLOAT4X4();
			for (int m = 0; m < 4; m++) {
				for (int n = 0; n < 4; n++) {
					g_vBuf0[i].f[j].m[m][n] = static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
					g_vBuf1[i].f[j].m[m][n] = static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
				}
			}
		}
    }
                      ...
                      UINT X, UINT Y, UINT Z )
{
    ...
    pd3dImmediateContext->Dispatch( X / 256, Y, Z );
